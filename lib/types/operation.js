  ...
  for (i = 0; i < this.parameters.length; i++) {
    ...
    if (param.type === 'array') {
      ...
      if (typeof param.allowableValues === 'undefined') {
		...
		if( param.in == 'body' && (!this.consumes || this.consumes.length == 0) ){
			param.signature = 'binary';
			param.sampleJSON = 'DATA';
		} else {
			param.signature = this.getModelSignature(innerType, this.models).toString();
			param.sampleJSON = Model.schemaToJSON(param, this.models, {}); //this.getModelSampleJSON(innerType, this.models);
	...
	for(var currentResponseCode in this.responses){
		if(this.responses[currentResponseCode].schema){
			...
			} else if(this.produces.length == 1 && this.produces[0] == 'application/octet-stream') {
				this.responses[currentResponseCode].signature = 'binary';
