  ...
  for (i = 0; i < this.parameters.length; i++) {
    ...
    if (param.type === 'array') {
      ...
      if (typeof param.allowableValues === 'undefined') {
		...
		// OutSystems change: support binary data-type for body parameters
		if( param.in == 'body' && !param.schema.format && (!this.consumes || this.consumes.length == 0) ){
			param.signature = 'binary';
			param.sampleJSON = 'DATA';
		} else {
			param.signature = this.getModelSignature(innerType, this.models).toString();
			if( param.signature == 'binary' && !param.schema.example && !param.schema.default){
				param.sampleJSON = 'DATA';
			} else {
				param.sampleJSON = Model.schemaToJSON(param, this.models, {});
			}
			param.responseClassSignature = param.signature;

			// OutSystems change: Get signature in the same manner as responses do
			if( param.schema && (param.schema.type == 'array' || param.schema.$ref)  ){			
				param.modelSignature = this.resolveModel(param.schema, definitions);
				if( param.modelSignature == null ){
					param.modelSignature = new Model(undefined, param.schema || {}, this.models);
				}
				param.signature = param.modelSignature.getMockSignature();
				param.sampleJSON = param.modelSignature.getSampleValue();
			}
		}
	...
	// OutSystems change: create response models
	for(var currentResponseCode in this.responses){
		if(this.responses[currentResponseCode].schema){
			var resolvedModel = this.resolveModel(this.responses[currentResponseCode].schema, definitions);
			if (resolvedModel){
				this.responses[currentResponseCode].resolvedModel = resolvedModel;
			} else if (!this.responses[currentResponseCode].schema.type || this.responses[currentResponseCode].schema.type === 'object' || this.responses[currentResponseCode].schema.type === 'array') {
				this.responses[currentResponseCode].resolvedModel = new Model(undefined, this.responses[currentResponseCode].schema || {}, this.models);
			}

			if( this.responses[currentResponseCode].resolvedModel ){
				this.responses[currentResponseCode].sampleJSON = this.responses[currentResponseCode].resolvedModel.createJSONSample();
				this.responses[currentResponseCode].signature = this.responses[currentResponseCode].resolvedModel.getMockSignature();
			} else	{
				var innerType = this.getType(this.responses[currentResponseCode]);
				this.responses[currentResponseCode].signature = this.getModelSignature(innerType, this.models).toString();
				if( this.responses[currentResponseCode].signature == 'binary' && 
						!this.responses[currentResponseCode].schema.example && 
						!this.responses[currentResponseCode].schema.default ){
							this.responses[currentResponseCode].sampleJSON = 'DATA';
						} else {
							this.responses[currentResponseCode].sampleJSON = Model.schemaToJSON(this.responses[currentResponseCode].schema, this.models, {});
						}
			}
		}
	}

	// OutSystems change: success responses are regular responses
...
Operation.prototype.getType = function (param) {
	...
	// OutSystems change: the name of the type should be its format
	if (type === 'integer') {
		if (format) {
			str = format;
		} else {
...
Operation.prototype.getModelSignature = function (type, definitions) {
  ...
  if (type instanceof Array) {
	...
	if (type.indexOf('#/definitions/') === 0){
		type = helpers.simpleRef(type);
	}

  if (type === 'string') {
    isPrimitive = true;
  } else {
    isPrimitive = (listType && definitions[type]) || (definitions[type]) ? false : true;
  }
...
// OutSystems change: Request Url function
